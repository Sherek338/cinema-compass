...
export default function MediaDetail() {
  ...
  const { isAuthenticated, user, updateWatchlist, updateFavorites } = useAuth();
  ...
  const inWatchlist = user?.watchList?.includes(movieId) || false;
  const inFavorites = user?.favoriteList?.includes(movieId) || false;
  ...
  const handleListChange = async (listType, action) => {
    ...
    try {
      ...
      if (listType === 'watchlist') {
        await updateWatchlist(movieId, action);
      } else if (listType === 'favorites') {
        await updateFavorites(movieId, action);
      }
    } catch (err) {
    ...
    return (
      <div className="flex justify-center items-center min-h-screen bg-[#201E1F] text-white">
        <div className="text-center">
          <p className="mb-4">Content not found.</p>
          <Link to="/" className="text-[#FF4002] hover:underline">
            Go back home
          </Link>
        </div>
      </div>
        ...
        <div className="flex flex-col lg:flex-row gap-10">
          ...
          <div className="flex flex-col justify-center gap-4">
            ...
            <div className="text-[#bbb] flex flex-wrap gap-4 text-sm">
              <p>
                <strong>Year:</strong> {displayYear || 'N/A'}
              </p>
              {!isSeries && runtime && (
                <p>
            ...
            {overview && (
              <p className="text-[#ccc] leading-relaxed">{overview}</p>
            )}
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              {genres?.map((g) => (
                <span
                  key={g.id}
                  className="border border-[#FF4002] text-[#FF4002] text-sm px-3 py-1 rounded-full"
                >
                  {g.name}
                </span>
              ))}
            ...
            {isAuthenticated && (
              <div className="flex flex-wrap gap-3 mt-6">
                <button
                  ...
                  disabled={updating}
                  className={`px-5 py-2 rounded-lg font-semibold transition disabled:opacity-50 disabled:cursor-not-allowed ${
                    inWatchlist
                      ? 'bg-gray-600 hover:bg-gray-700 text-white'
                      : 'bg-[#FF4002] hover:bg-[#ff5722] text-white'
                  }`}
                >
                  {updating
                    ? 'Updating...'
            ...
            <Link
              to={backLink}
              className="mt-6 inline-block text-[#999] hover:text-white transition"
            >
